...
open class VertxWebHandlersKoinModules : KoinModule {
    private val vertxWebHandlersOrderedKoinModule = module {
        ...
        single(VERTX_HTTPSERVER_SESSION_HANDLER.qualifier) {
            val store: SessionStore = ClusteredSessionStore.create(get<Vertx>(VERTX_INSTANCE.qualifier))
        ...
        single(VERTX_HTTPSERVER_OAUTH_PASSWORD_AUTHENTICATION_HANDLER.qualifier) {
            val oAuth2Auth = OAuth2Auth.create(get<Vertx>(VERTX_INSTANCE.qualifier), OAuth2Options().apply { flow = PASSWORD })
        ...
        single(VERTX_HTTPSERVER_JWT_AUTHENTICATION_HANDLER.qualifier) {
            val jwtAuth = JWTAuth.create(get<Vertx>(VERTX_INSTANCE.qualifier), JWTAuthOptions().apply { })
    ...
    }.toModuleWithOrder(moduleName = "vertxWebHandlersOrderedKoinModule")

    override val koinOrderedModules: LinkedHashSet<ModuleWithOrder> = linkedSetOf(vertxWebHandlersOrderedKoinModule)
}
